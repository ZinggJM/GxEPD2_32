...
GxEPD2_32_3C_X::GxEPD2_32_3C_X(GxEPD_Panel panel, int8_t cs, int8_t dc, int8_t rst, int8_t busy) :
  Adafruit_GFX(GxEPD2_32_3C_X_ScreenDimensions[panel].width, GxEPD2_32_3C_X_ScreenDimensions[panel].height),
...
void GxEPD2_32_3C_X::drawPixel(int16_t x, int16_t y, uint16_t color)
{
...
void GxEPD2_32_3C_X::fillScreen(uint16_t color)
{
  uint8_t black = 0x00;
  uint8_t red = 0x00;
...
void GxEPD2_32_3C_X::setFullWindow()
{
  _using_partial_mode = false;
}

void GxEPD2_32_3C_X::setPartialWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
...
void GxEPD2_32_3C_X::powerOff(void)
{
  _PowerOff();
}
...
bool GxEPD2_32_3C_X::_nextPagePart()
{
  uint16_t page_ys = _current_page * _page_height;
...
bool GxEPD2_32_3C_X::_nextPageFull154()
{
  uint16_t page_ys = _current_page * _page_height;
  uint16_t bytes = (_current_page < (_pages - 1) ? _page_height : HEIGHT - page_ys) * _width_bytes;
  if (!_second_phase)
...
bool GxEPD2_32_3C_X::_nextPageFull27()
{
  uint16_t page_ys = _current_page * _page_height;
  uint16_t bytes = (_current_page < (_pages - 1) ? _page_height : HEIGHT - page_ys) * _width_bytes;
  if (!_second_phase)
...
bool GxEPD2_32_3C_X::_nextPagePart27()
{
  uint16_t page_ys = _current_page * _page_height;
  uint16_t page_ye = _current_page < (_pages - 1) ? page_ys + _page_height : HEIGHT;
  uint16_t dest_ys = gx_uint16_max(_pw_y, page_ys);
...
bool GxEPD2_32_3C_X::_nextPageFull75()
{
  uint16_t page_ys = _current_page * _page_height;
  uint16_t bytes = (_current_page < (_pages - 1) ? _page_height : HEIGHT - page_ys) * _width_bytes;
  for (uint16_t idx = 0; idx < bytes; idx++)
...
bool GxEPD2_32_3C_X::_nextPagePart75()
{
  uint16_t page_ys = _current_page * _page_height;
  uint16_t page_ye = _current_page < (_pages - 1) ? page_ys + _page_height : HEIGHT;
  uint16_t dest_ys = gx_uint16_max(_pw_y, page_ys);
...
#if 1
void GxEPD2_32_3C_X::_send8pixel(uint8_t black_data, uint8_t red_data)
{
...
void GxEPD2_32_3C_X::_writeCommand(uint8_t c)
{
...
void GxEPD2_32_3C_X::_writeData(uint8_t d)
{
  if (_cs >= 0) digitalWrite(_cs, LOW);
  SPI.transfer(d);
  if (_cs >= 0) digitalWrite(_cs, HIGH);
...
void GxEPD2_32_3C_X::_writeData(const uint8_t* data, uint16_t n)
{
  if (_cs >= 0) digitalWrite(_cs, LOW);
...
void GxEPD2_32_3C_X::_writeData_nCS(const uint8_t* data, uint16_t n)
{
  for (uint8_t i = 0; i < n; i++)
  {
    if (_cs >= 0) digitalWrite(_cs, LOW);
...
void GxEPD2_32_3C_X::_waitWhileBusy(const char* comment)
{
...
void GxEPD2_32_3C_X::_setPartialRamArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
...
void GxEPD2_32_3C_X::_setPartialRamArea27(uint8_t command, uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
...
void GxEPD2_32_3C_X::_refreshWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  w = (w + 7 + (x % 8)) & 0xfff8; // byte boundary exclusive (round up)
...
void GxEPD2_32_3C_X::_PowerOn(void)
{
  if (!_power_is_on)
  {
    _writeCommand(0x04);
...
void GxEPD2_32_3C_X::_PowerOff(void)
{
  switch (_panel)
  {
    case GDEW0154Z04:
...
void GxEPD2_32_3C_X::_InitDisplay()
{
...
void GxEPD2_32_3C_X::_Init_Full()
{
  _InitDisplay();
  switch (_panel)
  {
...
void GxEPD2_32_3C_X::_Init_Part()
{
  _InitDisplay();
  switch (_panel)
  {
...
void GxEPD2_32_3C_X::_Update_Full(void)
{
  _writeCommand(0x12); //display refresh
  _waitWhileBusy("_Update_Full");
}

void GxEPD2_32_3C_X::_Update_Part(void)
{
  _writeCommand(0x12); //display refresh
  _waitWhileBusy("_Update_Part");
}

void GxEPD2_32_3C_X::_rotate(uint16_t& x, uint16_t& y, uint16_t& w, uint16_t& h)
{
  switch (getRotation())
  {
    case 1:
